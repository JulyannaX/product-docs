...
- 使用控制器控制摄像机旋转：开启时，玩家能使用摄像机滑动区控制摄像机旋转（一般情况不需要手动设置这个开关，推荐设置朝向模式）。如果您调整了摄像机朝向模式，编辑器会自动调整是否使用控制器控制摄像机旋转：使用摄像机朝向模式=固定朝向/跟随朝向时，会自动开启使用控制器控制摄像机旋转；使用摄像机朝向模式=控制朝向，会自动关闭使用控制器控制摄像机旋转。
